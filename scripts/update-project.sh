#!/usr/bin/env bash

# update-project.sh - Comprehensive project dependency and version updater
#
# This script provides a safe and comprehensive way to update:
# - Python version across all project files (pyproject.toml, Dockerfiles, workflows)
# - Python package dependencies with UV (with detailed change tracking)
# - UV package manager version in Dockerfiles and GitHub workflows
# - Node.js dependencies in frontend (package.json, package-lock.json)
# - Docker base images to latest versions
#
# Usage: ./scripts/update-project.sh [options]
#
# Options:
#   --python VERSION    Update Python version (default: keep current)
#   --node VERSION      Update Node.js version (default: keep current)
#   --no-backup        Skip creating backup files
#   --dry-run          Show what would be updated without making changes
#   --major            Include major version upgrades for packages
#   --skip-tests       Skip running tests after updates
#   --frontend-only    Only update frontend dependencies
#   --backend-only     Only update backend dependencies
#   --help             Show this help message

set -euo pipefail

# Default options
BACKUP=true
DRY_RUN=false
MAJOR_UPGRADES=false
SKIP_TESTS=false
UPDATE_PYTHON=false
UPDATE_NODE=false
PYTHON_VERSION=""
NODE_VERSION=""
FRONTEND_ONLY=false
BACKEND_ONLY=false
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
CHANGES_MADE=false

# Emojis for visual logging
EMOJI_INFO="ℹ️"
EMOJI_SUCCESS="✅"
EMOJI_WARNING="⚠️"
EMOJI_ERROR="❌"
EMOJI_ROCKET="🚀"
EMOJI_PACKAGE="📦"
EMOJI_PYTHON="🐍"
EMOJI_NODE="🟢"
EMOJI_DOCKER="🐳"
EMOJI_TEST="🧪"
EMOJI_BACKUP="💾"
EMOJI_CHANGES="📝"
EMOJI_VERIFY="🔍"
EMOJI_GIT="🔀"

# Print colored output with emojis
print_info() {
    echo -e "\033[0;34m$EMOJI_INFO  [INFO]\033[0m $1"
}

print_success() {
    echo -e "\033[0;32m$EMOJI_SUCCESS  [SUCCESS]\033[0m $1"
}

print_warning() {
    echo -e "\033[1;33m$EMOJI_WARNING  [WARNING]\033[0m $1"
}

print_error() {
    echo -e "\033[0;31m$EMOJI_ERROR  [ERROR]\033[0m $1"
}

print_section() {
    echo ""
    echo -e "\033[1;36m$1  $2\033[0m"
    echo -e "\033[1;36m$(printf '=%.0s' {1..60})\033[0m"
}

# Show usage
show_help() {
    head -n 25 "$0" | grep '^#' | sed 's/^# //' | sed 's/^#//'
    exit 0
}

# Parse command line arguments
while [[ $# -gt 0 ]]; do
    case $1 in
        --python)
            UPDATE_PYTHON=true
            PYTHON_VERSION="$2"
            shift 2
            ;;
        --node)
            UPDATE_NODE=true
            NODE_VERSION="$2"
            shift 2
            ;;
        --no-backup)
            BACKUP=false
            shift
            ;;
        --dry-run)
            DRY_RUN=true
            shift
            ;;
        --major)
            MAJOR_UPGRADES=true
            shift
            ;;
        --skip-tests)
            SKIP_TESTS=true
            shift
            ;;
        --frontend-only)
            FRONTEND_ONLY=true
            shift
            ;;
        --backend-only)
            BACKEND_ONLY=true
            shift
            ;;
        --help|-h)
            show_help
            ;;
        *)
            print_error "Unknown option: $1"
            show_help
            ;;
    esac
done

# Check if we're in the project root
if [[ ! -f "pyproject.toml" ]] || [[ ! -f "uv.lock" ]] || [[ ! -d "frontend" ]]; then
    print_error "This script must be run from the project root directory"
    exit 1
fi

# Check for uncommitted changes
if [[ -n $(git status --porcelain) ]]; then
    print_warning "You have uncommitted changes. Please commit or stash them first."
    print_info "This is required for safe rollback in case of issues."
    exit 1
fi

# Create backup directory
BACKUP_DIR="backups/project-updates-${TIMESTAMP}"
if [[ "$BACKUP" == true ]] && [[ "$DRY_RUN" == false ]]; then
    mkdir -p "$BACKUP_DIR"
    print_info "$EMOJI_BACKUP Creating backups in $BACKUP_DIR/"
fi

# Backup function
backup_file() {
    local file=$1
    if [[ "$BACKUP" == true ]] && [[ -f "$file" ]] && [[ "$DRY_RUN" == false ]]; then
        local backup_path
        backup_path="$BACKUP_DIR/$(dirname "$file")"
        mkdir -p "$backup_path"
        cp "$file" "$backup_path/$(basename "$file").backup"
    fi
}

# Track changes for summary
PACKAGE_CHANGES=()
NODE_PACKAGE_CHANGES=()
FILE_CHANGES=()
UV_VERSION_CHANGE=""
PYTHON_VERSION_CHANGE=""
NODE_VERSION_CHANGE=""
WORKFLOW_CHANGES=()

# Helper function to safely get array length
array_length() {
    local array_name=$1
    eval "echo \${#${array_name}[@]}" 2>/dev/null || echo 0
}

# Update Python version function
update_python_version() {
    if [[ "$UPDATE_PYTHON" != true ]] || [[ "$BACKEND_ONLY" == false && "$FRONTEND_ONLY" == true ]]; then
        return
    fi

    print_section "$EMOJI_PYTHON" "Updating Python Version"

    local current_version
    current_version=$(grep 'requires-python = ">=' pyproject.toml | sed 's/.*>=\([0-9.]*\)".*/\1/')
    PYTHON_VERSION_CHANGE="$current_version → $PYTHON_VERSION"

    if [[ "$current_version" == "$PYTHON_VERSION" ]]; then
        print_info "Python version is already $PYTHON_VERSION"
        return
    fi

    print_info "Updating Python from $current_version to $PYTHON_VERSION"

    if [[ "$DRY_RUN" == false ]]; then
        # Update pyproject.toml files
        for pyproject in pyproject.toml */pyproject.toml; do
            if [[ -f "$pyproject" ]]; then
                backup_file "$pyproject"
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s/requires-python = \">=.*/requires-python = \">=$PYTHON_VERSION\"/" "$pyproject"
                else
                    sed -i "s/requires-python = \">=.*/requires-python = \">=$PYTHON_VERSION\"/" "$pyproject"
                fi
                print_success "Updated $pyproject"
                FILE_CHANGES+=("$pyproject: Python $current_version → $PYTHON_VERSION")
            fi
        done

        # Update Dockerfiles
        for dockerfile in */Dockerfile; do
            if [[ -f "$dockerfile" ]] && grep -q "FROM.*python:" "$dockerfile"; then
                backup_file "$dockerfile"
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s/FROM.*python:[0-9.]*/FROM python:$PYTHON_VERSION/" "$dockerfile"
                else
                    sed -i "s/FROM.*python:[0-9.]*/FROM python:$PYTHON_VERSION/" "$dockerfile"
                fi
                print_success "Updated $dockerfile"
                FILE_CHANGES+=("$dockerfile: Python $current_version → $PYTHON_VERSION")
            fi
        done

        # Update GitHub workflows
        for workflow in .github/workflows/*.yml; do
            if [[ -f "$workflow" ]] && grep -q "python-version:" "$workflow"; then
                backup_file "$workflow"
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s/python-version: .*/python-version: \"$PYTHON_VERSION\"/" "$workflow"
                else
                    sed -i "s/python-version: .*/python-version: \"$PYTHON_VERSION\"/" "$workflow"
                fi
                print_success "Updated $workflow"
                WORKFLOW_CHANGES+=("$(basename "$workflow"): Python $current_version → $PYTHON_VERSION")
            fi
        done

        CHANGES_MADE=true
    else
        print_info "[DRY RUN] Would update Python version to $PYTHON_VERSION"
    fi
}

# Update Node.js version function
update_node_version() {
    if [[ "$UPDATE_NODE" != true ]] || [[ "$FRONTEND_ONLY" == false && "$BACKEND_ONLY" == true ]]; then
        return
    fi

    print_section "$EMOJI_NODE" "Updating Node.js Version"

    # Get current Node version from frontend Dockerfile
    local current_version=""
    if [[ -f "frontend/Dockerfile" ]]; then
        current_version=$(grep "FROM node:" frontend/Dockerfile | head -1 | sed -E 's/.*node:([0-9.]+).*/\1/')
    fi

    if [[ -z "$current_version" ]]; then
        print_warning "Could not determine current Node.js version"
        return
    fi

    NODE_VERSION_CHANGE="$current_version → $NODE_VERSION"

    if [[ "$current_version" == "$NODE_VERSION" ]]; then
        print_info "Node.js version is already $NODE_VERSION"
        return
    fi

    print_info "Updating Node.js from $current_version to $NODE_VERSION"

    if [[ "$DRY_RUN" == false ]]; then
        # Update frontend Dockerfile
        if [[ -f "frontend/Dockerfile" ]]; then
            backup_file "frontend/Dockerfile"
            if [[ "$OSTYPE" == "darwin"* ]]; then
                sed -i '' "s/FROM node:[0-9.]*/FROM node:$NODE_VERSION/" "frontend/Dockerfile"
            else
                sed -i "s/FROM node:[0-9.]*/FROM node:$NODE_VERSION/" "frontend/Dockerfile"
            fi
            print_success "Updated frontend/Dockerfile"
            FILE_CHANGES+=("frontend/Dockerfile: Node.js $current_version → $NODE_VERSION")
        fi

        # Update .nvmrc if exists
        if [[ -f "frontend/.nvmrc" ]]; then
            backup_file "frontend/.nvmrc"
            echo "$NODE_VERSION" > "frontend/.nvmrc"
            print_success "Updated frontend/.nvmrc"
            FILE_CHANGES+=("frontend/.nvmrc: Node.js $current_version → $NODE_VERSION")
        fi

        # Update GitHub workflows that use Node
        for workflow in .github/workflows/*.yml; do
            if [[ -f "$workflow" ]] && grep -q "node-version:" "$workflow"; then
                backup_file "$workflow"
                if [[ "$OSTYPE" == "darwin"* ]]; then
                    sed -i '' "s/node-version: .*/node-version: \"$NODE_VERSION\"/" "$workflow"
                else
                    sed -i "s/node-version: .*/node-version: \"$NODE_VERSION\"/" "$workflow"
                fi
                print_success "Updated $workflow"
                WORKFLOW_CHANGES+=("$(basename "$workflow"): Node.js $current_version → $NODE_VERSION")
            fi
        done

        CHANGES_MADE=true
    else
        print_info "[DRY RUN] Would update Node.js version to $NODE_VERSION"
    fi
}

# Update UV version in Dockerfiles and workflows
update_uv_version() {
    if [[ "$BACKEND_ONLY" == false && "$FRONTEND_ONLY" == true ]]; then
        return
    fi

    print_section "$EMOJI_DOCKER" "Updating UV Version"

    # Check required tools
    if ! command -v jq &> /dev/null; then
        print_warning "jq is not installed. Skipping UV version update."
        return
    fi

    # Get the latest UV version from GitHub
    local latest_uv
    latest_uv=$(curl -s https://api.github.com/repos/astral-sh/uv/releases/latest | jq -r '.tag_name' | sed 's/^v//')

    if [[ -z "$latest_uv" ]]; then
        print_warning "Could not determine latest UV version from GitHub"
        return
    fi

    print_info "Latest UV version: $latest_uv"

    # Get latest setup-uv action version
    local latest_setup_uv
    latest_setup_uv=$(curl -s https://api.github.com/repos/astral-sh/setup-uv/releases/latest | jq -r '.tag_name')
    local latest_setup_uv_commit
    latest_setup_uv_commit=$(curl -s https://api.github.com/repos/astral-sh/setup-uv/commits/$latest_setup_uv | jq -r '.sha')

    print_info "Latest setup-uv action: $latest_setup_uv (commit: ${latest_setup_uv_commit:0:7})"

    # Find current UV version in Dockerfiles
    local current_uv=""
    for dockerfile in */Dockerfile; do
        if [[ -f "$dockerfile" ]]; then
            local version
            version=$(grep "ghcr.io/astral-sh/uv:" "$dockerfile" 2>/dev/null | head -1 | sed -E 's/.*uv:([0-9.]+).*/\1/')
            if [[ -n "$version" ]]; then
                current_uv="$version"
                break
            fi
        fi
    done

    # Update Dockerfiles
    if [[ -n "$current_uv" ]] && [[ "$current_uv" != "$latest_uv" ]]; then
        UV_VERSION_CHANGE="$current_uv → $latest_uv"
        print_info "Updating UV from $current_uv to $latest_uv in Dockerfiles"

        if [[ "$DRY_RUN" == false ]]; then
            for dockerfile in */Dockerfile; do
                if [[ -f "$dockerfile" ]] && grep -q "ghcr.io/astral-sh/uv:" "$dockerfile"; then
                    backup_file "$dockerfile"
                    if [[ "$OSTYPE" == "darwin"* ]]; then
                        sed -i '' "s/ghcr.io\/astral-sh\/uv:[0-9.]*[0-9]/ghcr.io\/astral-sh\/uv:$latest_uv/g" "$dockerfile"
                    else
                        sed -i "s/ghcr.io\/astral-sh\/uv:[0-9.]\+/ghcr.io\/astral-sh\/uv:$latest_uv/g" "$dockerfile"
                    fi
                    print_success "Updated $dockerfile"
                    FILE_CHANGES+=("$dockerfile: UV $current_uv → $latest_uv")
                    CHANGES_MADE=true
                fi
            done
        else
            print_info "[DRY RUN] Would update UV version in Dockerfiles"
        fi
    else
        print_success "UV version in Dockerfiles is already up to date ($current_uv)"
    fi

    # Update GitHub workflows that use setup-uv action
    print_info "Checking GitHub workflows for setup-uv updates..."

    for workflow in .github/workflows/*.yml; do
        if [[ -f "$workflow" ]] && grep -q "astral-sh/setup-uv@" "$workflow"; then
            local current_commit
            current_commit=$(grep -oE "astral-sh/setup-uv@[a-f0-9]+" "$workflow" | head -1 | cut -d'@' -f2)

            if [[ -n "$current_commit" ]] && [[ "$current_commit" != "$latest_setup_uv_commit" ]]; then
                print_info "Updating setup-uv in $(basename "$workflow")"

                if [[ "$DRY_RUN" == false ]]; then
                    backup_file "$workflow"

                    if [[ "$OSTYPE" == "darwin"* ]]; then
                        sed -i '' "s/astral-sh\/setup-uv@[a-f0-9]\{40\}/astral-sh\/setup-uv@$latest_setup_uv_commit/g" "$workflow"
                        sed -i '' "s/# v[0-9.]\+/# $latest_setup_uv/g" "$workflow"
                    else
                        sed -i "s/astral-sh\/setup-uv@[a-f0-9]\{40\}/astral-sh\/setup-uv@$latest_setup_uv_commit/g" "$workflow"
                        sed -i "s/# v[0-9.]\+/# $latest_setup_uv/g" "$workflow"
                    fi

                    print_success "Updated $(basename "$workflow")"
                    WORKFLOW_CHANGES+=("$(basename "$workflow"): setup-uv ${current_commit:0:7} → ${latest_setup_uv_commit:0:7}")
                    CHANGES_MADE=true
                else
                    print_info "[DRY RUN] Would update setup-uv in $(basename "$workflow")"
                fi
            fi
        fi
    done

    if [[ $(array_length WORKFLOW_CHANGES) -eq 0 ]]; then
        print_success "GitHub workflows are already up to date"
    fi
}

# Capture Python package changes
capture_python_package_changes() {
    if [[ "$DRY_RUN" == true ]]; then
        return
    fi

    # Compare uv.lock before and after
    if [[ -f "$BACKUP_DIR/uv.lock.backup" ]]; then
        print_info "$EMOJI_CHANGES Analyzing Python package changes..."

        # Extract package versions from backup
        local old_packages
        old_packages=$(grep -E "^name = |^version = " "$BACKUP_DIR/uv.lock.backup" | paste -d' ' - - | sed 's/name = "\(.*\)" version = "\(.*\)"/\1==\2/')

        # Extract package versions from current
        local new_packages
        new_packages=$(grep -E "^name = |^version = " "uv.lock" | paste -d' ' - - | sed 's/name = "\(.*\)" version = "\(.*\)"/\1==\2/')

        # Find changes
        while IFS= read -r old_pkg; do
            local pkg_name
            pkg_name=$(echo "$old_pkg" | cut -d'=' -f1)
            local old_version
            old_version=$(echo "$old_pkg" | cut -d'=' -f3)

            local new_version
            new_version=$(echo "$new_packages" | grep "^$pkg_name==" | cut -d'=' -f3 || echo "")

            if [[ -n "$new_version" ]] && [[ "$old_version" != "$new_version" ]]; then
                PACKAGE_CHANGES+=("$pkg_name: $old_version → $new_version")
                CHANGES_MADE=true
            fi
        done <<< "$old_packages"
    fi
}

# Update Python packages
update_python_packages() {
    if [[ "$BACKEND_ONLY" == false && "$FRONTEND_ONLY" == true ]]; then
        return
    fi

    print_section "$EMOJI_PACKAGE" "Updating Python Packages"

    # Check for uv
    if ! command -v uv &> /dev/null; then
        print_error "uv is not installed. Please install it first."
        print_info "Visit: https://github.com/astral-sh/uv"
        exit 1
    fi

    # Backup critical files
    if [[ "$BACKUP" == true ]] && [[ "$DRY_RUN" == false ]]; then
        backup_file "uv.lock"
        backup_file "pyproject.toml"

        for service in ingestor populator; do
            if [[ -f "$service/pyproject.toml" ]]; then
                backup_file "$service/pyproject.toml"
            fi
        done
    fi

    # Update uv itself
    print_info "Checking for uv updates..."
    if [[ "$DRY_RUN" == false ]]; then
        uv self update || print_warning "Could not update uv"
    else
        print_info "[DRY RUN] Would check for uv updates"
    fi

    # Compile dependencies with upgrades
    print_info "Compiling upgraded dependencies..."

    local uv_cmd="uv lock"
    if [[ "$MAJOR_UPGRADES" == true ]]; then
        uv_cmd="$uv_cmd --upgrade"
        print_info "Including major version upgrades"
    else
        uv_cmd="$uv_cmd --upgrade"
        print_info "Upgrading to latest minor/patch versions"
    fi

    if [[ "$DRY_RUN" == true ]]; then
        print_info "[DRY RUN] Would run: $uv_cmd"
        print_info "Checking for available updates..."
        uv tree --outdated || true
    else
        if $uv_cmd; then
            print_success "Dependencies compiled successfully"
            CHANGES_MADE=true
        else
            print_error "Failed to compile dependencies"
            exit 1
        fi
    fi

    # Sync to install upgraded packages
    if [[ "$DRY_RUN" == false ]]; then
        print_info "Syncing upgraded dependencies..."
        if uv sync --all-extras; then
            print_success "Dependencies synced successfully"
        else
            print_error "Failed to sync dependencies"
            exit 1
        fi

        # Capture package changes
        capture_python_package_changes
    else
        print_info "[DRY RUN] Would run: uv sync --all-extras"
    fi
}

# Capture Node package changes
capture_node_package_changes() {
    if [[ "$DRY_RUN" == true ]]; then
        return
    fi

    # Compare package-lock.json before and after
    if [[ -f "$BACKUP_DIR/frontend/package-lock.json.backup" ]]; then
        print_info "$EMOJI_CHANGES Analyzing Node.js package changes..."

        # Extract package versions using jq
        if command -v jq &> /dev/null; then
            local old_packages
            old_packages=$(jq -r '.packages | to_entries | .[] | select(.key != "") | "\(.key | sub("node_modules/"; ""))=\(.value.version)"' "$BACKUP_DIR/frontend/package-lock.json.backup" 2>/dev/null | sort)

            local new_packages
            new_packages=$(jq -r '.packages | to_entries | .[] | select(.key != "") | "\(.key | sub("node_modules/"; ""))=\(.value.version)"' "frontend/package-lock.json" 2>/dev/null | sort)

            # Find changes
            while IFS= read -r old_pkg; do
                local pkg_name
                pkg_name=$(echo "$old_pkg" | cut -d'=' -f1)
                local old_version
                old_version=$(echo "$old_pkg" | cut -d'=' -f2)

                local new_version
                new_version=$(echo "$new_packages" | grep "^$pkg_name=" | cut -d'=' -f2 || echo "")

                if [[ -n "$new_version" ]] && [[ "$old_version" != "$new_version" ]]; then
                    NODE_PACKAGE_CHANGES+=("$pkg_name: $old_version → $new_version")
                    CHANGES_MADE=true
                fi
            done <<< "$old_packages"
        else
            print_warning "jq not available, skipping detailed package change analysis"
        fi
    fi
}

# Update Node.js packages
update_node_packages() {
    if [[ "$FRONTEND_ONLY" == false && "$BACKEND_ONLY" == true ]]; then
        return
    fi

    print_section "$EMOJI_NODE" "Updating Node.js Packages"

    # Check for npm
    if ! command -v npm &> /dev/null; then
        print_error "npm is not installed. Please install Node.js first."
        exit 1
    fi

    # Change to frontend directory
    cd frontend

    # Backup package files
    if [[ "$BACKUP" == true ]] && [[ "$DRY_RUN" == false ]]; then
        backup_file "package.json"
        backup_file "package-lock.json"
    fi

    # Check for outdated packages
    print_info "Checking for outdated packages..."
    npm outdated || true

    if [[ "$DRY_RUN" == false ]]; then
        # Update packages
        if [[ "$MAJOR_UPGRADES" == true ]]; then
            print_info "Updating all packages (including major versions)..."
            npm update --save
            npx npm-check-updates -u
            npm install
        else
            print_info "Updating packages (minor and patch versions only)..."
            npm update --save
        fi

        print_success "Node.js packages updated successfully"
        CHANGES_MADE=true

        # Capture package changes
        capture_node_package_changes
    else
        print_info "[DRY RUN] Would update Node.js packages"
        if [[ "$MAJOR_UPGRADES" == true ]]; then
            print_info "[DRY RUN] Would include major version updates"
            npx npm-check-updates || true
        fi
    fi

    # Return to project root
    cd ..
}

# Run tests
run_tests() {
    if [[ "$SKIP_TESTS" == true ]] || [[ "$DRY_RUN" == true ]]; then
        return
    fi

    print_section "$EMOJI_TEST" "Running Tests"

    local test_failed=false

    # Run Python tests if backend was updated
    if [[ "$FRONTEND_ONLY" != true ]]; then
        print_info "Running Python linters..."
        if uv run task lint; then
            print_success "Python linting passed"
        else
            print_warning "Python linting failed - review the changes"
            test_failed=true
        fi

        print_info "Running Python tests..."
        if uv run task test; then
            print_success "Python tests passed"
        else
            print_warning "Python tests failed - review the changes"
            test_failed=true
        fi
    fi

    # Run frontend tests if frontend was updated
    if [[ "$BACKEND_ONLY" != true ]]; then
        print_info "Running frontend linters..."
        cd frontend
        if npm run lint; then
            print_success "Frontend linting passed"
        else
            print_warning "Frontend linting failed - review the changes"
            test_failed=true
        fi

        print_info "Running frontend tests..."
        if npm test; then
            print_success "Frontend tests passed"
        else
            print_warning "Frontend tests failed - review the changes"
            test_failed=true
        fi
        cd ..
    fi

    if [[ "$test_failed" == true ]]; then
        print_warning "Some tests failed. Please review the changes before committing."
    fi
}

# Generate summary
generate_summary() {
    print_section "$EMOJI_CHANGES" "Update Summary"

    if [[ "$DRY_RUN" == true ]]; then
        print_info "This was a dry run. No changes were made."
        print_info "Run without --dry-run to apply changes."
        return
    fi

    if [[ "$CHANGES_MADE" == false ]]; then
        print_success "Everything is already up to date! No changes were needed."
        return
    fi

    # Python version changes
    if [[ -n "$PYTHON_VERSION_CHANGE" ]]; then
        echo ""
        echo "🐍 Python Version:"
        echo "  $PYTHON_VERSION_CHANGE"
    fi

    # Node.js version changes
    if [[ -n "$NODE_VERSION_CHANGE" ]]; then
        echo ""
        echo "🟢 Node.js Version:"
        echo "  $NODE_VERSION_CHANGE"
    fi

    # UV version changes
    if [[ -n "$UV_VERSION_CHANGE" ]]; then
        echo ""
        echo "🐳 UV Package Manager:"
        echo "  $UV_VERSION_CHANGE"
    fi

    # Python package changes
    if [[ $(array_length PACKAGE_CHANGES) -gt 0 ]]; then
        echo ""
        echo "📦 Python Package Updates:"
        printf '%s\n' "${PACKAGE_CHANGES[@]:-}" | sort | while IFS= read -r change; do
            echo "  • $change"
        done
    fi

    # Node package changes
    if [[ $(array_length NODE_PACKAGE_CHANGES) -gt 0 ]]; then
        echo ""
        echo "📦 Node.js Package Updates:"
        # Show only first 10 changes if many
        if [[ $(array_length NODE_PACKAGE_CHANGES) -gt 10 ]]; then
            printf '%s\n' "${NODE_PACKAGE_CHANGES[@]:0:10}" | sort | while IFS= read -r change; do
                echo "  • $change"
            done
            echo "  ... and $(($(array_length NODE_PACKAGE_CHANGES) - 10)) more"
        else
            printf '%s\n' "${NODE_PACKAGE_CHANGES[@]:-}" | sort | while IFS= read -r change; do
                echo "  • $change"
            done
        fi
    fi

    # File changes
    if [[ $(array_length FILE_CHANGES) -gt 0 ]]; then
        echo ""
        echo "📄 File Updates:"
        printf '%s\n' "${FILE_CHANGES[@]:-}" | sort | while IFS= read -r change; do
            echo "  • $change"
        done
    fi

    # Workflow changes
    if [[ $(array_length WORKFLOW_CHANGES) -gt 0 ]]; then
        echo ""
        echo "🔄 GitHub Workflow Updates:"
        for change in "${WORKFLOW_CHANGES[@]:-}"; do
            echo "  • $change"
        done
    fi

    # Git instructions
    echo ""
    print_section "$EMOJI_GIT" "Next Steps"

    echo "1. Review the changes:"
    echo "   git diff --stat"

    if [[ $(array_length PACKAGE_CHANGES) -gt 0 ]]; then
        echo "   git diff uv.lock"
    fi

    if [[ $(array_length NODE_PACKAGE_CHANGES) -gt 0 ]]; then
        echo "   git diff frontend/package-lock.json"
    fi

    if [[ -n "$UV_VERSION_CHANGE" ]] || [[ -n "$PYTHON_VERSION_CHANGE" ]] || [[ -n "$NODE_VERSION_CHANGE" ]]; then
        echo "   git diff --name-only | grep -E '(Dockerfile|\.yml)'"
    fi

    echo ""
    echo "2. Stage the changes:"

    # Backend changes
    if [[ "$FRONTEND_ONLY" != true ]]; then
        if [[ $(array_length PACKAGE_CHANGES) -gt 0 ]]; then
            echo "   git add uv.lock"
        fi

        if [[ -n "$PYTHON_VERSION_CHANGE" ]]; then
            echo "   git add pyproject.toml */pyproject.toml"
            echo "   git add .github/workflows/*.yml"
        fi

        if [[ -n "$UV_VERSION_CHANGE" ]]; then
            echo "   git add */Dockerfile"
        fi
    fi

    # Frontend changes
    if [[ "$BACKEND_ONLY" != true ]]; then
        if [[ $(array_length NODE_PACKAGE_CHANGES) -gt 0 ]]; then
            echo "   git add frontend/package.json frontend/package-lock.json"
        fi

        if [[ -n "$NODE_VERSION_CHANGE" ]]; then
            echo "   git add frontend/Dockerfile"
            echo "   git add frontend/.nvmrc"
        fi
    fi

    if [[ $(array_length WORKFLOW_CHANGES) -gt 0 ]]; then
        echo "   git add .github/workflows/*.yml"
    fi

    echo ""
    echo "3. Commit the changes:"
    echo "   git commit -m \"chore: update dependencies"

    if [[ -n "$PYTHON_VERSION_CHANGE" ]]; then
        echo ""
        echo "   - Update Python to ${PYTHON_VERSION_CHANGE##* → }"
    fi

    if [[ -n "$NODE_VERSION_CHANGE" ]]; then
        echo "   - Update Node.js to ${NODE_VERSION_CHANGE##* → }"
    fi

    if [[ -n "$UV_VERSION_CHANGE" ]]; then
        echo "   - Update UV to ${UV_VERSION_CHANGE##* → }"
    fi

    if [[ $(array_length PACKAGE_CHANGES) -gt 0 ]]; then
        echo "   - Update $(array_length PACKAGE_CHANGES) Python packages"
    fi

    if [[ $(array_length NODE_PACKAGE_CHANGES) -gt 0 ]]; then
        echo "   - Update $(array_length NODE_PACKAGE_CHANGES) Node.js packages"
    fi

    echo "   \""
}

# Manual verification steps
show_verification_steps() {
    print_section "$EMOJI_VERIFY" "Manual Verification Steps"

    echo "Please verify the following before merging:"
    echo ""
    echo "1. 🐳 Docker builds:"
    echo "   docker-compose build --no-cache"
    echo ""
    echo "2. 🧪 Service health checks:"
    echo "   docker-compose up -d"
    echo "   docker-compose ps  # All services should be 'healthy'"
    echo ""
    echo "3. 🔍 Smoke tests:"
    if [[ "$BACKEND_ONLY" != true ]]; then
        echo "   # Check frontend"
        echo "   curl -f http://localhost:5173"
    fi
    if [[ "$FRONTEND_ONLY" != true ]]; then
        echo "   # Check that services start"
        echo "   docker-compose logs -f"
    fi
    echo ""
    echo "4. 📊 Review dependency changes:"
    if [[ "$FRONTEND_ONLY" != true ]]; then
        echo "   # Check for Python security advisories"
        echo "   uv pip audit"
    fi
    if [[ "$BACKEND_ONLY" != true ]]; then
        echo "   # Check for Node.js security advisories"
        echo "   cd frontend && npm audit"
    fi
    echo ""
    echo "5. 📝 Update CHANGELOG.md if needed"
    echo ""

    if [[ "$BACKUP" == true ]]; then
        echo "💾 Backups are stored in: $BACKUP_DIR/"
        echo "   To restore:"
        if [[ "$FRONTEND_ONLY" != true ]]; then
            echo "   cp $BACKUP_DIR/uv.lock.backup uv.lock && uv sync --all-extras"
        fi
        if [[ "$BACKEND_ONLY" != true ]]; then
            echo "   cp $BACKUP_DIR/frontend/package-lock.json.backup frontend/package-lock.json && cd frontend && npm ci"
        fi
    fi
}

# Handle errors
trap 'handle_error $?' ERR

handle_error() {
    local exit_code=$1
    print_error "An error occurred (exit code: $exit_code)"

    if [[ "$BACKUP" == true ]] && [[ "$DRY_RUN" == false ]] && [[ -d "$BACKUP_DIR" ]]; then
        print_info "You can restore from backup with:"
        if [[ -f "$BACKUP_DIR/uv.lock.backup" ]]; then
            echo "  cp $BACKUP_DIR/uv.lock.backup uv.lock"
            echo "  uv sync --all-extras"
        fi
        if [[ -f "$BACKUP_DIR/frontend/package-lock.json.backup" ]]; then
            echo "  cp $BACKUP_DIR/frontend/package-lock.json.backup frontend/package-lock.json"
            echo "  cd frontend && npm ci"
        fi
    fi

    exit $exit_code
}

# Main execution
main() {
    print_section "$EMOJI_ROCKET" "Starting Project Update"

    # Check if conflicting options are set
    if [[ "$FRONTEND_ONLY" == true && "$BACKEND_ONLY" == true ]]; then
        print_error "Cannot use both --frontend-only and --backend-only"
        exit 1
    fi

    # Update Python version if requested
    update_python_version

    # Update Node.js version if requested
    update_node_version

    # Update UV version in Dockerfiles
    update_uv_version

    # Update Python packages
    update_python_packages

    # Update Node.js packages
    update_node_packages

    # Run tests
    run_tests

    # Generate summary
    generate_summary

    # Show verification steps
    if [[ "$DRY_RUN" == false ]] && [[ "$CHANGES_MADE" == true ]]; then
        show_verification_steps
    fi
}

# Run main function
main
